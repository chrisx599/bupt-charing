...
class FastCharger(Charger):
    def __init__(self) -> None:
        super().__init__()
        self.power_per_hour = 30


class SlowCharger(Charger):
    def __init__(self) -> None:
        super().__init__()
        self.power_per_hour = 7
...
class ChargeSystem(threading.Thread):
    ...
    def update_wait_station(self):
        ...
        if not self.fast_wait_area_queue.empty():
            for item in self.fast_wait_area_queue:
        ...
        for item in self.fast_charger:
            ...
            if not item.use_state and item.charger_state:
                ...
                if not item.queue.empty():
                    # 从充电桩的等待队列中选取第一个充电
                    current_car = item.queue[0]
                    current_car = Car()  # TODO:方便写代码
                    # 记录当前开始充电时间
                    current_car.start_charge_time = self.timer.get_time()
                    # 计算充电所需要的时间
...
class SimulateTimer():
    def __init__(self) -> None:
        self.timer = threading.Timer(2, self.run())  # 每2s模拟1分钟,每30s模拟1小时
